...
size_t xm_scalar_sizeof(int type)
{
  static const size_t tbl[] = {
      sizeof(float), sizeof(float complex), sizeof(double), sizeof(double complex),
  };
...
void xm_scalar_axpy(xm_scalar_t a, void *x, const void *y, size_t len, int type)
{
  size_t i;
  ...
  switch (type) {
    case XM_SCALAR_FLOAT: {
      float *xx = x;
      const float *yy = y;
      for (i = 0; i < len; i++) xx[i] = a * xx[i] + yy[i];
      return;
    }
    case XM_SCALAR_FLOAT_COMPLEX: {
      float complex *xx = x;
