...
xm_block_space_t *xm_block_space_create(xm_dim_t dims)
{
  ...
  if ((ret = calloc(1, sizeof *ret)) == NULL) return NULL;
  ret->dims = dims;
...
xm_block_space_t *xm_block_space_clone(const xm_block_space_t *bs)
{
  ...
  for (i = 0; i < ret->dims.n; i++) {
    ...
    memcpy(ret->splits[i], bs->splits[i], (ret->nblocks.i[i] + 1) * sizeof(size_t));
  }
  return ret;
}

xm_block_space_t *xm_block_space_permute_clone(const xm_block_space_t *bs, xm_dim_t permutation)
{
  xm_block_space_t *ret;
  size_t i, j;
  ...
  for (j = 0; j < ret->dims.n; j++) {
    ...
    memcpy(ret->splits[i], bs->splits[j], (ret->nblocks.i[i] + 1) * sizeof(size_t));
  }
  return ret;
}

size_t xm_block_space_get_ndims(const xm_block_space_t *bs) { return bs->dims.n; }

xm_dim_t xm_block_space_get_abs_dims(const xm_block_space_t *bs) { return bs->dims; }

xm_dim_t xm_block_space_get_nblocks(const xm_block_space_t *bs) { return bs->nblocks; }
...
void xm_block_space_split(xm_block_space_t *bs, size_t dim, size_t x)
{
  ...
  for (i = 0; i < bs->nblocks.i[dim] + 1; i++) {
    if (bs->splits[dim][i] > x) {
      if ((nptr = realloc(bs->splits[dim], (bs->nblocks.i[dim] + 2) * sizeof(size_t))) == NULL)
        return;
      bs->splits[dim] = nptr;
      break;
...
xm_dim_t xm_block_space_get_block_dims(const xm_block_space_t *bs, xm_dim_t blkidx)
{
  xm_dim_t blkdims;
  size_t i;
  ...
  for (i = 0; i < blkdims.n; i++) {
    blkdims.i[i] = bs->splits[i][blkidx.i[i] + 1] - bs->splits[i][blkidx.i[i]];
  }
  return blkdims;
}

size_t xm_block_space_get_block_size(const xm_block_space_t *bs, xm_dim_t blkidx)
{
  size_t i, size = 1;
  ...
  for (i = 0; i < bs->dims.n; i++) {
    size *= bs->splits[i][blkidx.i[i] + 1] - bs->splits[i][blkidx.i[i]];
  }
  return size;
}
...
void xm_block_space_decompose_index(const xm_block_space_t *bs, xm_dim_t idx, xm_dim_t *blkidx,
                                    xm_dim_t *elidx)
{
  size_t i, j;
...
int xm_block_space_eq(const xm_block_space_t *bsa, const xm_block_space_t *bsb)
{
  ...
  if (bsa->dims.n != bsb->dims.n) return 0;
  for (i = 0; i < bsa->dims.n; i++)
    if (!xm_block_space_eq1(bsa, i, bsb, i)) return 0;
  return 1;
}

int xm_block_space_eq1(const xm_block_space_t *bsa, size_t dima, const xm_block_space_t *bsb,
                       size_t dimb)
{
  size_t i;
  ...
  if (bsa->nblocks.i[dima] != bsb->nblocks.i[dimb]) return 0;
  for (i = 0; i < bsa->nblocks.i[dima] + 1; i++)
    if (bsa->splits[dima][i] != bsb->splits[dimb][i]) return 0;
  return 1;
}
...
void xm_block_space_free(xm_block_space_t *bs)
{
  ...
  if (bs) {
    for (i = 0; i < bs->dims.n; i++) free(bs->splits[i]);
    free(bs);
  }
}
