...
void sgemm_(char *, char *, long int *, long int *, long int *, float *, float *, long int *,
            float *, long int *, float *, float *, long int *);
void cgemm_(char *, char *, long int *, long int *, long int *, float complex *, float complex *,
            long int *, float complex *, long int *, float complex *, float complex *, long int *);
void dgemm_(char *, char *, long int *, long int *, long int *, double *, double *, long int *,
            double *, long int *, double *, double *, long int *);
void zgemm_(char *, char *, long int *, long int *, long int *, double complex *, double complex *,
            long int *, double complex *, long int *, double complex *, double complex *,
            long int *);

static void xgemm(char transa, char transb, long int m, long int n, long int k, xm_scalar_t alpha,
                  void *a, long int lda, void *b, long int ldb, xm_scalar_t beta, void *c,
                  long int ldc, int type)
{
  switch (type) {
...
static void compute_block(xm_scalar_t alpha, const xm_tensor_t *a, const xm_tensor_t *b,
                          ...
                          xm_dim_t blkidxc, struct blockpair *pairs, void *buf)
{
  ...
  for (i = 0; i < nblkk; i++) {
    if (pairs[i].alpha == 0) continue;
    for (j = i + 1; j < nblkk; j++) {
      xm_dim_t dia, dja, dib, djb, pia, pja, pib, pjb;
      size_t ii, good = 1;
      if (pairs[j].alpha == 0) continue;
      dia = pairs[i].blkidxa;
      dja = pairs[j].blkidxa;
      dib = pairs[i].blkidxb;
      djb = pairs[j].blkidxb;
      if (xm_tensor_get_block_data_ptr(a, dia) != xm_tensor_get_block_data_ptr(a, dja) ||
          xm_tensor_get_block_data_ptr(b, dib) != xm_tensor_get_block_data_ptr(b, djb))
        continue;
      ...
      for (ii = 0; ii < aidxa.n && good; ii++) {
        if (pia.i[aidxa.i[ii]] != pja.i[aidxa.i[ii]]) good = 0;
      }
      for (ii = 0; ii < aidxb.n && good; ii++) {
        if (pib.i[aidxb.i[ii]] != pjb.i[aidxb.i[ii]]) good = 0;
      }
      if (good) {
  ...
  for (i = 0; i < nblkk; i++) {
    if (pairs[i].alpha != 0) {
      ...
      if (aidxc.n > 0 && aidxc.i[0] == 0) {
        xgemm('T', 'N', (int)n, (int)m, (int)k, alpha * pairs[i].alpha, bufb2, (int)k, bufa2,
              (int)k, 1.0, bufc1, (int)n, type);
      } else {
        xgemm('T', 'N', (int)m, (int)n, (int)k, alpha * pairs[i].alpha, bufa2, (int)k, bufb2,
...
void xm_contract(xm_scalar_t alpha, const xm_tensor_t *a, const xm_tensor_t *b, xm_scalar_t beta,
                 xm_tensor_t *c, const char *idxa, const char *idxb, const char *idxc)
{
  ...
  if (strlen(idxa) != xm_block_space_get_ndims(bsa)) fatal("bad contraction indices");
  if (strlen(idxb) != xm_block_space_get_ndims(bsb)) fatal("bad contraction indices");
  if (strlen(idxc) != xm_block_space_get_ndims(bsc)) fatal("bad contraction indices");

  xm_make_masks(idxa, idxb, &cidxa, &cidxb);
  xm_make_masks(idxc, idxa, &cidxc, &aidxa);
  xm_make_masks(idxc, idxb, &aidxc, &aidxb);

  if (aidxa.n + cidxa.n != xm_block_space_get_ndims(bsa)) fatal("bad contraction indices");
  if (aidxb.n + cidxb.n != xm_block_space_get_ndims(bsb)) fatal("bad contraction indices");
  if (aidxc.n + cidxc.n != xm_block_space_get_ndims(bsc)) fatal("bad contraction indices");
  ...
  nblocksa = xm_tensor_get_nblocks(a);
  nblkk = xm_dim_dot_mask(&nblocksa, &cidxa);
  bufbytes = 2 * (xm_tensor_get_largest_block_bytes(a) + xm_tensor_get_largest_block_bytes(b) +
...
#pragma omp parallel private(i)
#endif
  {
    ...
    if ((pairs = malloc(nblkk * sizeof *pairs)) == NULL) fatal("out of memory");
    if ((buf = malloc(bufbytes)) == NULL) fatal("out of memory");
#ifdef _OPENMP
#pragma omp for schedule(dynamic)
#endif
    for (i = 0; i < nblklist; i++) {
      if ((int)i % mpisize == mpirank)
        compute_block(alpha, a, b, beta, c, cidxa, aidxa, cidxb, aidxb, cidxc, aidxc, blklist[i],
                      pairs, buf);
    }
    free(buf);
    free(pairs);
