...
{
  if (npages >= 65536) fatal("allocation size is too large");
  return (offset | (npages << 48));
}

static size_t get_block_offset(uint64_t data_ptr) { return (data_ptr & ((1ULL << 48) - 1)); }

static size_t get_block_npages(uint64_t data_ptr) { return (data_ptr >> 48); }
...
static int extend_file(xm_allocator_t *allocator)
{
  ...
  if (allocator->file_bytes % (XM_PAGE_SIZE * 8)) oldsize++;
  ...
  if (allocator->file_bytes % (XM_PAGE_SIZE * 8)) newsize++;
...
static uint64_t find_pages(xm_allocator_t *allocator, size_t n_pages)
{
  ...
  for (start = 0; start < n_total; start++)
    if (!bitmap_test(allocator->pages, start)) break;
  if (start == n_total) return (XM_NULL_PTR);
  for (i = start, n_free = 0; i < n_total; i++) {
    n_free = bitmap_test(allocator->pages, i) ? 0 : n_free + 1;
    if (n_free == n_pages) {
      uint64_t offset;
      for (offset = i + 1 - n_free; offset <= i; offset++) bitmap_set(allocator->pages, offset);
...
xm_allocator_t *xm_allocator_create(const char *path)
{
  ...
  if (path) {
    ...
    if ((allocator->pages = calloc(1, 1)) == NULL) fatal("out of memory");
    if (allocator->mpirank == 0) {
      if ((allocator->fd = open(path, O_CREAT | O_RDWR, S_IRUSR | S_IWUSR)) == -1) {
        perror("open");
        free(allocator);
        return (NULL);
      }
      if (ftruncate(allocator->fd, (off_t)allocator->file_bytes)) {
        perror("ftruncate");
        if (close(allocator->fd)) perror("close");
        free(allocator);
        return (NULL);
      }
#ifdef XM_USE_MPI
...
const char *xm_allocator_get_path(xm_allocator_t *allocator) { return (allocator->path); }
...
void xm_allocator_read(xm_allocator_t *allocator, uint64_t data_ptr, void *mem, size_t size_bytes)
{
  ssize_t read_bytes;
  off_t offset;
  ...
  if (read_bytes != (ssize_t)size_bytes) fatal("pread");
}

void xm_allocator_write(xm_allocator_t *allocator, uint64_t data_ptr, const void *mem,
                        size_t size_bytes)
{
  ssize_t write_bytes;
  off_t offset;
  ...
  if (write_bytes != (ssize_t)size_bytes) fatal("pwrite");
}

void xm_allocator_deallocate(xm_allocator_t *allocator, uint64_t data_ptr)
{
  if (allocator->mpirank != 0) return;
  if (data_ptr == XM_NULL_PTR) return;
#ifdef _OPENMP
  omp_set_lock(&allocator->mutex);
#endif
  if (allocator->path) {
    ...
    for (i = 0; i < npages; i++) bitmap_clear(allocator->pages, start + i);
  } else {
    free((void *)data_ptr);
  }
#ifdef _OPENMP
...
void xm_allocator_destroy(xm_allocator_t *allocator)
{
  ...
  if (allocator->mpirank == 0) {
    if (allocator->path) {
      if (close(allocator->fd)) perror("close");
      if (unlink(allocator->path)) perror("unlink");
    }
  }
#ifdef _OPENMP
