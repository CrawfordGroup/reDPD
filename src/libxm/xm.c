...
#endif
  if ((buf = malloc(xm_tensor_get_largest_block_bytes(a))) == NULL) fatal("out of memory");
  maxblksize = xm_tensor_get_largest_block_size(a);
  switch (xm_tensor_get_scalar_type(a)) {
    case XM_SCALAR_FLOAT:
      for (i = 0; i < maxblksize; i++) ((float *)buf)[i] = (float)x;
      break;
    case XM_SCALAR_FLOAT_COMPLEX:
      for (i = 0; i < maxblksize; i++) ((float complex *)buf)[i] = (float complex)x;
      break;
    case XM_SCALAR_DOUBLE:
      for (i = 0; i < maxblksize; i++) ((double *)buf)[i] = (double)x;
      break;
    case XM_SCALAR_DOUBLE_COMPLEX:
      for (i = 0; i < maxblksize; i++) ((double complex *)buf)[i] = (double complex)x;
      break;
  }
...
#pragma omp for schedule(dynamic)
    ...
    for (i = 0; i < nblklist; i++) {
      if ((int)i % mpisize == mpirank) xm_tensor_write_block(a, blklist[i], buf);
    }
  }
  free(buf);
  free(blklist);
...
void xm_copy(xm_tensor_t *a, xm_scalar_t s, const xm_tensor_t *b, const char *idxa,
             const char *idxb)
{
  xm_add(0, a, s, b, idxa, idxb);
}

void xm_add(xm_scalar_t alpha, xm_tensor_t *a, xm_scalar_t beta, const xm_tensor_t *b,
            const char *idxa, const char *idxb)
{
  ...
  if (strlen(idxa) != xm_block_space_get_ndims(bsa)) fatal("idxa does not match tensor dimensions");
  if (strlen(idxb) != xm_block_space_get_ndims(bsb)) fatal("idxb does not match tensor dimensions");
  xm_make_masks(idxa, idxb, &cidxa, &cidxb);
  if (cidxa.n != xm_block_space_get_ndims(bsa) || cidxb.n != xm_block_space_get_ndims(bsb))
    fatal("index spaces do not match");
  for (i = 0; i < cidxa.n; i++)
    if (!xm_block_space_eq1(bsa, cidxa.i[i], bsb, cidxb.i[i])) fatal("inconsistent block-spaces");
...
#pragma omp parallel private(i)
#endif
  {
    ...
    if ((buf1 = malloc(maxblkbytes)) == NULL) fatal("out of memory");
    if ((buf2 = malloc(maxblkbytes)) == NULL) fatal("out of memory");
    ...
    for (i = 0; i < nblklist; i++) {
      if ((int)i % mpisize == mpirank) {
        ...
        if (beta == 0 || blocktype == XM_BLOCK_TYPE_ZERO) {
          ...
          xm_scalar_mul(buf1, blksize, scalartype, scalar);
        ...
        if (alpha == 0)
          ...
          xm_scalar_axpy(alpha, buf1, buf2, blksize, scalartype);
