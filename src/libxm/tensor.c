...
static struct xm_block *tensor_get_block(const xm_tensor_t *tensor, xm_dim_t blkidx)
{
  xm_dim_t nblocks;
  size_t offset;
...
xm_tensor_t *xm_tensor_create(const xm_block_space_t *bs, int type, xm_allocator_t *allocator)
{
  ...
  if (type != XM_SCALAR_FLOAT && type != XM_SCALAR_FLOAT_COMPLEX && type != XM_SCALAR_DOUBLE &&
      type != XM_SCALAR_DOUBLE_COMPLEX)
    fatal("unknown scalar type");
  if ((ret = calloc(1, sizeof *ret)) == NULL) fatal("out of memory");
  if ((ret->bs = xm_block_space_clone(bs)) == NULL) fatal("out of memory");
  ret->type = type;
  ret->allocator = allocator;
  nblocks = xm_block_space_get_nblocks(bs);
  if ((ret->blocks = calloc(xm_dim_dot(&nblocks), sizeof *ret->blocks)) == NULL)
    fatal("out of memory");
                                        ...
                                        xm_allocator_t *allocator)
{
  ...
  if (allocator == NULL) allocator = xm_tensor_get_allocator(tensor);
  ...
  while (xm_dim_ne(&idx, &nblocks)) {
    ...
    if (ret->blocks[i].type == XM_BLOCK_TYPE_CANONICAL) xm_tensor_set_canonical_block(ret, idx);
    xm_dim_inc(&idx, &nblocks);
  }
  return ret;
}

const xm_block_space_t *xm_tensor_get_block_space(const xm_tensor_t *tensor) { return tensor->bs; }

int xm_tensor_get_scalar_type(const xm_tensor_t *tensor) { return tensor->type; }

xm_allocator_t *xm_tensor_get_allocator(const xm_tensor_t *tensor) { return tensor->allocator; }
...
xm_scalar_t xm_tensor_get_element(const xm_tensor_t *tensor, xm_dim_t idx)
{
  ...
  if (block->type == XM_BLOCK_TYPE_ZERO) return 0;
  ...
  if ((buf = malloc(blkbytes)) == NULL) fatal("out of memory");
...
{
  return xm_tensor_get_block_size(tensor, blkidx) * xm_scalar_sizeof(tensor->type);
}
...
{
  return xm_tensor_get_largest_block_size(tensor) * xm_scalar_sizeof(tensor->type);
}

uint64_t xm_tensor_get_block_data_ptr(const xm_tensor_t *tensor, xm_dim_t blkidx)
{
  struct xm_block *block;

  block = tensor_get_block(tensor, blkidx);
  if (block->type == XM_BLOCK_TYPE_CANONICAL) return block->data_ptr;
  if (block->type == XM_BLOCK_TYPE_DERIVATIVE) return tensor->blocks[block->data_ptr].data_ptr;
  return XM_NULL_PTR;
}

xm_dim_t xm_tensor_get_block_permutation(const xm_tensor_t *tensor, xm_dim_t blkidx)
{
  return tensor_get_block(tensor, blkidx)->permutation;
}

xm_scalar_t xm_tensor_get_block_scalar(const xm_tensor_t *tensor, xm_dim_t blkidx)
{
...
void xm_tensor_set_canonical_block(xm_tensor_t *tensor, xm_dim_t blkidx)
{
  ...
  if (data_ptr == XM_NULL_PTR) fatal("unable to allocate block data");
  xm_tensor_set_canonical_block_raw(tensor, blkidx, data_ptr);
}

void xm_tensor_set_canonical_block_raw(xm_tensor_t *tensor, xm_dim_t blkidx, uint64_t data_ptr)
{
  struct xm_block *block;

  if (data_ptr == XM_NULL_PTR) fatal("unexpected null data pointer");
  block = tensor_get_block(tensor, blkidx);
...
void xm_tensor_set_derivative_block(xm_tensor_t *tensor, xm_dim_t blkidx, xm_dim_t source_blkidx,
                                    xm_dim_t permutation, xm_scalar_t scalar)
{
  struct xm_block *block;
  ...
  if (xm_dim_ne(&blkdims1, &blkdims2)) fatal("invalid block permutation");
  nblocks = xm_tensor_get_nblocks(tensor);
  block = tensor_get_block(tensor, blkidx);
...
void xm_tensor_get_canonical_block_list(const xm_tensor_t *tensor, xm_dim_t **blklist,
                                        size_t *nblklist)
{
  ...
  while (xm_dim_ne(&idx, &nblocks)) {
    ...
    if (blocktype == XM_BLOCK_TYPE_CANONICAL) {
      ...
      if (list == NULL) fatal("out of memory");
      list[nlist - 1] = idx;
    }
    xm_dim_inc(&idx, &nblocks);
  }
...
void xm_tensor_read_block(const xm_tensor_t *tensor, xm_dim_t blkidx, void *buf)
{
  size_t blkbytes;
  uint64_t data_ptr;
  int blocktype;
  ...
  if (blocktype == XM_BLOCK_TYPE_ZERO) fatal("cannot read data from zero-blocks");
  blkbytes = xm_tensor_get_block_bytes(tensor, blkidx);
  data_ptr = xm_tensor_get_block_data_ptr(tensor, blkidx);
  xm_allocator_read(tensor->allocator, data_ptr, buf, blkbytes);
}

void xm_tensor_write_block(xm_tensor_t *tensor, xm_dim_t blkidx, const void *buf)
{
  size_t blkbytes;
  uint64_t data_ptr;
  int blocktype;
  ...
  blocktype = xm_tensor_get_block_type(tensor, blkidx);
  if (blocktype != XM_BLOCK_TYPE_CANONICAL) fatal("can only write to canonical blocks");
  blkbytes = xm_tensor_get_block_bytes(tensor, blkidx);
  data_ptr = xm_tensor_get_block_data_ptr(tensor, blkidx);
...
#define DEFINE_UNFOLD_FUNCTION(_type)                                                             \
  static void xm_tensor_unfold_block_##_type(const xm_tensor_t *tensor, xm_dim_t blkidx,          \
                                             xm_dim_t mask_i, xm_dim_t mask_j, const _type *from, \
                                             _type *to, size_t stride)                            \
  {                                                                                               \
    ...
    if (mask_i.n > 0) {                                                                           \
      lead_ii = mask_i.i[0];                                                                      \
      for (kk = 0; kk < permutation.i[lead_ii]; kk++) inc *= blkdimsp.i[kk];                      \
      for (ii = 0; ii < mask_i.n - 1; ii++) mask_i.i[ii] = mask_i.i[ii + 1];                      \
      mask_i.n--;                                                                                 \
    ...
    if (inc == 1) { /* use memcpy */                                                              \
      for (jj = 0; jj < block_size_j; jj++) {                                                     \
        ...
        for (ii = 0; ii < block_size_i; ii += lead_ii_nel) {                                      \
          ...
          memcpy(&to[jj * stride + ii], from + offset, sizeof(_type) * lead_ii_nel);              \
      ...
      for (jj = 0; jj < block_size_j; jj++) {                                                     \
        ...
        for (ii = 0; ii < block_size_i; ii += lead_ii_nel) {                                      \
          ...
          for (kk = 0; kk < lead_ii_nel; kk++) {                                                  \
            to[jj * stride + ii + kk] = from[offset];                                             \
            offset += inc;                                                                        \
          }                                                                                       \
          xm_dim_inc_mask(&elidx, &blkdims, &mask_i);                                             \
        }                                                                                         \
...
void xm_tensor_unfold_block(const xm_tensor_t *tensor, xm_dim_t blkidx, xm_dim_t mask_i,
                            xm_dim_t mask_j, const void *from, void *to, size_t stride)
{
  if (from == NULL || to == NULL || from == to) fatal("invalid argument");
  if (mask_i.n + mask_j.n != blkidx.n) fatal("invalid mask dimensions");

  switch (tensor->type) {
    case XM_SCALAR_FLOAT:
      xm_tensor_unfold_block_float(tensor, blkidx, mask_i, mask_j, from, to, stride);
      return;
    case XM_SCALAR_FLOAT_COMPLEX:
      xm_tensor_unfold_block_float_complex(tensor, blkidx, mask_i, mask_j, from, to, stride);
      return;
    case XM_SCALAR_DOUBLE:
      xm_tensor_unfold_block_double(tensor, blkidx, mask_i, mask_j, from, to, stride);
      return;
    case XM_SCALAR_DOUBLE_COMPLEX:
      xm_tensor_unfold_block_double_complex(tensor, blkidx, mask_i, mask_j, from, to, stride);
      return;
  }
}

#define DEFINE_FOLD_FUNCTION(_type)                                                             \
  static void xm_tensor_fold_block_##_type(const xm_tensor_t *tensor, xm_dim_t blkidx,          \
                                           xm_dim_t mask_i, xm_dim_t mask_j, const _type *from, \
                                           _type *to, size_t stride)                            \
  {                                                                                             \
    ...
    if (mask_i.n > 0) {                                                                         \
      lead_ii = mask_i.i[0];                                                                    \
      for (kk = 0; kk < lead_ii; kk++) inc *= blkdims.i[kk];                                    \
      for (ii = 0; ii < mask_i.n - 1; ii++) mask_i.i[ii] = mask_i.i[ii + 1];                    \
      mask_i.n--;                                                                               \
      lead_ii_nel = blkdims.i[lead_ii];                                                         \
    }                                                                                           \
    if (inc == 1) { /* use memcpy */                                                            \
      for (jj = 0; jj < block_size_j; jj++) {                                                   \
        ...
        for (ii = 0; ii < block_size_i; ii += lead_ii_nel) {                                    \
          ...
          memcpy(to + offset, &from[jj * stride + ii], sizeof(_type) * lead_ii_nel);            \
          xm_dim_inc_mask(&elidx, &blkdims, &mask_i);                                           \
        }                                                                                       \
        xm_dim_inc_mask(&elidx, &blkdims, &mask_j);                                             \
      }                                                                                         \
...
void xm_tensor_fold_block(const xm_tensor_t *tensor, xm_dim_t blkidx, xm_dim_t mask_i,
                          xm_dim_t mask_j, const void *from, void *to, size_t stride)
{
  int blocktype;
  ...
  blocktype = xm_tensor_get_block_type(tensor, blkidx);
  if (blocktype != XM_BLOCK_TYPE_CANONICAL) fatal("can only fold canonical blocks");

  switch (tensor->type) {
    case XM_SCALAR_FLOAT:
      xm_tensor_fold_block_float(tensor, blkidx, mask_i, mask_j, from, to, stride);
      return;
    case XM_SCALAR_FLOAT_COMPLEX:
      xm_tensor_fold_block_float_complex(tensor, blkidx, mask_i, mask_j, from, to, stride);
      return;
    case XM_SCALAR_DOUBLE:
      xm_tensor_fold_block_double(tensor, blkidx, mask_i, mask_j, from, to, stride);
      return;
    case XM_SCALAR_DOUBLE_COMPLEX:
      xm_tensor_fold_block_double_complex(tensor, blkidx, mask_i, mask_j, from, to, stride);
      return;
  }
}
